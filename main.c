  ...
  if (autoMode) {
    ...
    if (anyFlame) {
      ...
      if (flameMiddle) {
        ...
        statusMessage = "🔥 FIRE AHEAD! Moving forward...";
        ...
        statusMessage = "💦 Spraying water ahead!";
        Serial.println(statusMessage);
        ...
        // LEFT FIRE DETECTED
        statusMessage = "🔥 FIRE LEFT! Turning left...";
        Serial.println(statusMessage);
        ...
        // Spray water with servo sweep
        statusMessage = "💦 Spraying water on left!";
        Serial.println(statusMessage);
        digitalWrite(PUMP_RELAY, LOW);  // Pump ON
        servoSweep4Times();
        digitalWrite(PUMP_RELAY, HIGH); // Pump OFF
        ...
        // RIGHT FIRE DETECTED
        statusMessage = "🔥 FIRE RIGHT! Turning right...";
        Serial.println(statusMessage);
        ...
        // Spray water with servo sweep
        statusMessage = "💦 Spraying water on right!";
        Serial.println(statusMessage);
        digitalWrite(PUMP_RELAY, LOW);  // Pump ON
        servoSweep4Times();
        digitalWrite(PUMP_RELAY, HIGH); // Pump OFF
    ...
    if (gasDetected && !gasWasDetected) {
      ...
      fanTimerActive = true;
      gasWasDetected = true;
      statusMessage = "💨 SMOKE DETECTED! Fan ON for 15 seconds";
      Serial.println(statusMessage);
    }
    ...
    if (fanTimerActive && (millis() - fanStartTime >= 1000)) {
      ...
      statusMessage = "✓ Smoke cleared. Fan stopped.";
      Serial.println(statusMessage);
    }
...
<body>
    <div class='header'>
        <h1>🔥 FIREFIGHTING ROBOT CONTROL 🚒</h1>
    ...
    <div class='container'>
        ...
        <div style='text-align: center; margin: 30px 0; color: rgba(255,255,255,0.5); font-size: 0.9em;'>
            <p>🤖 IoT Firefighting Robot v2.0</p>
            <p>Real-time monitoring • Auto-refresh every 500ms</p>
        </div>
    </div>
        ...
        function updateStatus() {
            fetch('/status')
                ...
                .catch(error => {
                    ...
                    document.getElementById('statusMsg').innerText = '⚠️ Connection Lost';
                });
        }
...
void handleMove() {
  ...
  if (server.hasArg("dir")) {
    ...
    if (direction == "forward") {
      moveForward();
      statusMessage = "⬆️ Moving Forward";
    } else if (direction == "backward") {
      moveBackward();
      statusMessage = "⬇️ Moving Backward";
    } else if (direction == "left") {
      turnLeft();
      statusMessage = "⬅️ Turning Left";
    } else if (direction == "right") {
      turnRight();
      statusMessage = "➡️ Turning Right";
    }
...
void handlePumpOn() {
  ...
  statusMessage = "💧 Pump manually activated";
...
void handlePumpOff() {
  ...
  digitalWrite(PUMP_RELAY, HIGH);  // OFF
  statusMessage = "💧 Pump manually deactivated";
...
void handleFanOn() {
  ...
  statusMessage = "💨 Fan manually activated";
...
void handleFanOff() {
  ...
  digitalWrite(FAN_RELAY, HIGH);  // OFF
  statusMessage = "💨 Fan manually deactivated";
...
void handleMode() {
  ...
  if (autoMode) {
    ...
    digitalWrite(PUMP_RELAY, HIGH);  // OFF
    digitalWrite(FAN_RELAY, HIGH);   // OFF
    statusMessage = "🤖 AUTO MODE - Robot will fight fires automatically";
    server.send(200, "text/plain", "AUTO MODE");
  } else {
    stopMotors();
    statusMessage = "🎮 MANUAL MODE - You have full control";
